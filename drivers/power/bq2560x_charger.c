...
struct bq2560x {
	...
	struct wake_lock 	chg_plugin_wake_lock;
...
static irqreturn_t bq2560x_charger_interrupt(int irq, void *dev_id)
{
	...
	if (bq->is_lock_init == false) {
		wake_lock_init(&bq->chg_plugin_wake_lock, WAKE_LOCK_SUSPEND, "chg_plugin_lock");
